...
const everyModel = fs.readdirSync(everyModelPath).filter(file => file.startsWith('co2'))

// Copying the origin model file to the root of the package (for use in others publicodes projects)
fs.copyFileSync(path.join(process.cwd(), `${everyModelFolder}/${originModelFile}`), destPath)
...
everyModel.forEach(model => {
  ...
  fs.writeFileSync(destPath+'/index.d.ts', generateTypes(`${everyModelFolder}/${model}`))
...
function generateIndex(pathToModel) {
  return `
import rules from "${pathToModel}" assert { type: "json" };
